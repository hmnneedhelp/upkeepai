...
from src.db.session import RoofingMaterial, SeriesOfProjects, WallMaterial, AccidentRate, RoofCleaning, HousingStock, MkdStatus, MkdManagementStatus, MkdCategory, Mkd

file = pd.ExcelFile('/app/backend/src/db_fill/mkd.xlsx')
...
async def fill_COL_781(session: AsyncSession):
    ...
    for value in a.values[1:]:
        ...
        await session.execute(insert(RoofingMaterial).values(id=value[0], name=value[1]))
...
async def fill_COL_758(session: AsyncSession):
    ...
    for value in a.values[1:]:
        print(value[0], value[1])
        await session.execute(insert(SeriesOfProjects).values(id=value[0], name=value[1]))
    await session.commit()
...
async def fill_COL_3243(session: AsyncSession):
    ...
    for value in a.values[1:]:
        print(value[0], value[1])
        await session.execute(insert(MkdManagementStatus).values(id=value[0], name=value[1]))
    await session.commit()
...
async def fill_main_column(session: AsyncSession):
    ...
    for value in a.values[3:]:
        # print(value[1], value[2])
        # await session.execute(insert(Mkd).values(
        ...
        await session.execute(insert(Mkd).values(
            ...
            num_passenger_elevators=int(value[20]) if not pd.isnull(value[20]) else None,
            num_freight_passenger_elevators=int(value[21]) if not pd.isnull(value[21]) else None,
            roof_cleaning_id=int(value[22]) if not pd.isnull(value[22]) else None,
            roofing_material_id=int(value[23]) if not pd.isnull(value[23]) else None,
            ...
            mkd_management_status_id=int(value[28]) if not pd.isnull(value[28]) else None,
            num_freight_elevators=int(value[29]) if not pd.isnull(value[29]) else None,
            reason_for_status_change=value[30] if not pd.isnull(value[30]) else None,
