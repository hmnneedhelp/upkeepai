...
def train_num_inc_model(df_1: pd.DataFrame, df_2: pd.DataFrame):
    ...
    boosting_model.save_model("/app/backend/app/src/service/upkeep/model/num_inc_model")
...
def train_cap_works_model(df_1, df_3, df_4):
    ...
    categorical_features = df[cat_cols].applymap(float)
    ...
    # Сохранение модели
    model.save("/app/backend/app/src/service/upkeep/model/num_works_model.h5")
